...
export interface FichierCommentaire {
  ...
  taille?: number;
  dateUpload?: Date;
  commentaireProjet?:Commentaire;
  commentaireActivite?:Commentaire;
...
export class FichierCommentaireService {
  ...
  transformeElementType(elemntType: ELEMENTTYPE) {
    if (elemntType === 'PROJET') return 'commentaire-projet';
    if (elemntType === 'ACTIVITE') return 'commentaire-activite';
    if (elemntType === 'TACHE') return 'commentaire-tache';
    return '';
  }
